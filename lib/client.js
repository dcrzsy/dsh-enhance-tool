...
function boardEmptyState() {
	return { version: 1, updatedAt: 0, entries: {}, collapsed: [], hidden: [], undo: [] };
}
...
function BoardDialog() {
	...
	*/
	const purgeTrash = async (ids) => {
		setPurgeBusy(true);
		setPurgeNote("");
		const result = await boardDeleteSessions(ids);
		...
		boardCommit(boardUndoAdd(boardHide(state, ids), undoable));
		...
		setPurgeNote(result.unavailable
			...
			: ptL("已删除 " + result.deleted.length + " 个会话" + (result.failed.length > 0 ? "；" + result.failed.length + " 个未删除，已从看板移除（" + firstReason + "）" : "") + (undoable.length > 0 ? "【可从上方撤销】" : ""),
				"deleted " + result.deleted.length + (result.failed.length > 0 ? "; " + result.failed.length + " not deleted, removed from this board (" + firstReason + ")" : "") + (undoable.length > 0 ? " [undo available above]" : "")));
	};
	...
	return jsx("div", {
		...
		children: jsxs("div", {
			...
			children: [
				...
				jsx("div", { key: "hint", className: "enhc-board-hint", children: ptL("拖拽卡片可跨列移动；「⋯」或按 M 打开移动菜单。垃圾桶里的「清空」会经 dsh-archived-chats 真正删除这些会话（含日志，不可恢复）。", "Drag cards between columns; press M or ⋯ for the move menu. Emptying 垃圾桶 really deletes those sessions through dsh-archived-chats (logs included, irreversible).") })
			]
		})
	});
}
