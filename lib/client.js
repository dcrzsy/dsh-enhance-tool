...
function boardAddItem(state, title) {
	...
	next.items[id] = { title: text, done: false, at: Date.now() };
...
*/
function BoardItemCard({ card, ui, now, onMove, onDelete, onUpdate }) {
	...
	return jsxs("div", {
		...
		onClick: () => {
			if (ui.select === true) boardPatch({ picked: ui.picked.includes(card.id) ? ui.picked.filter((id) => id !== card.id) : [...ui.picked, card.id] });
		},
		onDoubleClick: (event) => {
...
function BoardDialog() {
	...
	const addItem = (title) => boardCommit(boardAddItem(state, title));
	...
	*/
	const purgeTrash = async (ids) => {
		...
		boardCommit(boardUndoAdd(boardHide(next, sessionIds), undoable));
								...
								children: cards.length === 0 && column.key !== "todo" ? jsx("div", { className: "enhc-board-empty", children: ptL("拖到此处", "Drop here") }) : [
									...visible.flatMap((card, index) => {
										...
										nodes.push(card.item !== void 0
											? jsx(BoardItemCard, { card, ui, now, onMove: move, onDelete: deleteItem, onUpdate: updateItem }, card.id)
