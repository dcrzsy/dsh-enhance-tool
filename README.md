...
| **任务看板（通用面板）** | 会话头部「轨迹」右侧的看板入口：待办 / 进行中 / 完成 / 归档 / 垃圾桶五列，卡片=会话，支持拖拽跨列与列内排序、批量选择、列折叠、搜索/工作区过滤、自动归类 | 设置 → 通用 → 通用面板 |
...
会话头部「轨迹」右侧的图标按钮打开一个宽模态看板，**卡片 = 会话**。这是给「一堆并行会话」做的人工分诊台，不改动任何会话数据。
...
- 其他：搜索（标题/路径）、工作区下拉、含子会话、显示全部、列折叠（持久化）、待办列「归档过期 N」（把超过窗口的待办一次性归档）、垃圾桶「清空」（两步确认）。
- 卡片徽标：`● 运行中`、`✓ 已完成`、`⏱ N 个后台任务`、`子会话`、`自动`（非手动放置）。
