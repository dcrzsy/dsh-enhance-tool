...
| **任务看板（通用面板）** | 会话头部「轨迹」右侧的「看板」页签式入口：待办 / 进行中 / 完成 / 归档 / 垃圾桶五列，卡片=会话，支持拖拽跨列与列内排序、批量选择、列折叠、搜索/工作区过滤、自动归类 | 设置 → 通用 → 通用面板 |
...
**五列与自动归类**（未被手动拖过的会话按下列规则自动落列，从上到下首个命中）

| 列 | 自动规则 |
|---|---|
| 进行中 | `running`，或该会话有 `running` / `stopping` 的后台任务 |
| 完成 | 这一轮已结束（`completed`） |
| 待办 | 最近 7 天内有更新（工具栏「显示全部」可放开这个窗口） |
| 归档 | 其余（更早的会话） |
| 垃圾桶 | **只有手动拖入**（不自动进） |
...
- 键盘 / 触屏兜底：卡片可聚焦，`Enter` 打开会话，`M` 或卡片「⋯」打开「移到…」菜单（五列 + 重置为自动）。
- 从侧边栏拖入：dsh 侧栏的行拖拽 payload 就是 `sessionId`（实测 `dataTransfer.getData("text/plain")`），所以**可以直接把侧边栏会话拖到某一列**。
- 点击卡片 = 打开该会话并关闭面板；`批量选择` 模式下点击 = 勾选，工具栏出现批量条（五列 + 清除）。
- 其他：搜索（标题/路径）、工作区下拉、含子会话、显示全部、列折叠（持久化，折叠后保留**竖排列名**）、待办列「归档过期 N」（把超过窗口的待办一次性归档）、垃圾桶「清空」（两步确认）。
- 卡片徽标：`● 运行中`、`✓ 已完成`、`⏱ N 个后台任务`、`子会话`、`· 手动`（只标记被手动固定的卡片——「自动」是默认态，不占位）。

**排版**（0.10.2 起）

- 面板 `1360×830`（随视口收缩），五列等宽（每列 ≥212px），列间距 10px、圆角 14px，沿用产品弹层底色/描边/字号变量。
